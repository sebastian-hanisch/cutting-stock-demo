...
def pattern_figure(problem, patterns, stock_idx, counts, title: str) -> go.Figure:
    ...
    visible per pattern, not just as a single aggregate percentage. Each
    pattern is cut from a specific stock type, so the bar's total length
    (and the label) vary with which stock type that pattern uses."""
    order = sorted((j for j in range(len(counts)) if counts[j] > 0), key=lambda j: -counts[j])
    y_labels = [
        f"Muster {rank + 1} ({problem.stock_types[stock_idx[j]].label}, ×{counts[j]})"
    ...
    waste_widths = [
        problem.stock_types[stock_idx[j]].length - float(sum(patterns[j] * problem.lengths)) for j in order
    ]
    fig.add_trace(go.Bar(
